...
clusters=struct('x',[],'y',[],'gtId',[],'imId',[],'patches',[],...
    'clusterId',[],'clusters',[]);
clusters.patches = false(radius*2+1,radius*2+1,9000*5*nImgs);%区块声明，每个图像每个（5==gt） 9000个样本块，每个大小为（radius*2+1）*（radius*2+1）
...
% clustering patcher for skech token - by Meng
if (0)
    h = radius*2+1;
    w = radius*2+1;
    ds = 200;
    % clustering based on Daisy Descriptor compute Daisy as paper said
    pn = length(clusters.patches);
    pf = [];%pf 每个区块特征向量
    for i = 1 : pn
        patch = clusters.patches(:,:,i);
        dzy = compute_daisy(pn);
        [y,x] = find(patch);
        nlables = length(x);
        for j = 1 : nlables
            feature = zeros(1,h*w*ds);
            feature(((y-1)*w+x-1)*ds+1:((y-1)*w+x)*ds) = dzy.descs((y-1)*w+x,:);
        end
        pf = [pf;feature];
    end
end
